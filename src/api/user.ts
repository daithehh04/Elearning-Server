...
userRouter.post(
  Endpoint.GET_USER_FROM_EMAIL_GOOGLE,
  asyncHandler(async (req, res) => {
    const { email } = <{ email: string }>req.body
    const users = await userService.checkUserFromEmailGoogle(email)
...
userRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const body: { token: string; password: string; newPassword: string } =
      req.body
...
userRouter.post(
  Endpoint.GET_TOTAL_LEARNED_TOPIC,
  asyncHandler(async (req, res) => {
...
userRouter.post(
  Endpoint.UPDATE_STUDYED_FOR_USER,
  asyncHandler(async (req, res) => {
