...
export default class TopicService {
  ...
  getTopicsByStatus = async (body: { status: number }): Promise<Topic[]> => {
    try {
      const topics = await TopicModel.find({ status: body.status }).populate(
        "topicChild"
      );
  ...
  getTopicsByCourse = async (body: {
    ...
    try {
      if (body.status) {
        ...
        data.forEach((o) => {
          o.topicChild.map(
            (c) => new Topic(c).status === body.status && total++
          );
        });
        return {
      ...
      try {
        ...
        if (topics) {
          ...
          if (parent) {
            ...
            if (
              !parentTopic?.topicChild.find(
                (o) => o.toString() == topic?.id?.toString()
              )
            ) {
  ...
  orderTopic = async (body: {
    ...
    try {
      ...
      const orderTopic = loadTopic
        .map((topic) => new Topic(topic))
        .map((topic) => {
          return {
            ...topic,
            index:
              body?.indexRange?.find((o) => o.id === topic.id?.toString())
                ?.index || 0,
          };
        });

      const data = await Promise.all(
        orderTopic.map((order) => {
